...
void processCommands(CourseNode *pHead, char *pszCommandFileName);
...
int main(int argc, char *argv[])
{
    ...
    pHead = getCourses(pszCourseFileName);
    printCourses("Initial Courses", pHead);
    
    sortCourses(&pHead);

    printCourses("Sorted Courses", pHead);

    // process the command input file
    processCommands(pHead, pszCommandFileName);

    // print the course after processing the commandfile
    printCourses("Resulting Courses", pHead);
    printf("\n");
    return 0;
}
    ...
    */
    while (fgets(szInputBuffer, 100, pfileCommand) != NULL)
    {
        ...
        {   // STUDENT command
            ...
            if (bGotToken)
                processStudentCommand(pHead, szSubCommand
        ...
        {   // COURSE command
            bGotToken = getSimpleToken(szInputBuffer, &iBufferPosition, szSubCommand);
            if (bGotToken)
                processCourseCommand(pHead, szSubCommand
